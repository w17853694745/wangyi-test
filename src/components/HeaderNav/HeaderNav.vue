  ...
  <header class="header_box">
    ...
    <div class="header_nav">
      ...
      <div class="right_span">
        ...
        <div class="img_box">
          <img :class="{is_open: isOpen}" @click="isOpen=!isOpen" :src="'//yanxuan-static.nosdn.127.net/hxm/yanxuan-wap/p/20161201/style/img/icon-normal/arrow-down-3-799ded53ea.png'"/>
        </div>
      </div>
      <div class="all_nav_box" v-show="isOpen">
        <p>全部频道</p>
        <ul class="all_nav clearfix" ref="allNav" @click="addActive($event)">
          <li v-for="(item, index) in shoppingList" :key="index" :class="{active: curA===index}"><a href="javascript:"  :data-this-index="index">{{item}}</a></li>
        </ul>
      </div>
    </div>
  </header>
